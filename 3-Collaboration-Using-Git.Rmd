...
Once you've made one or more commits - hopefully with informative commit messages - you can `push` those changesto whatever Git server you're using. Typically, you can simply use the command `git push` to push all new commits from your current branch to your default remote server. In some cases, you may need to specify the remote (here, "origin") and branch ("master") to push to.
